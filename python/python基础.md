...
import json
d = dict(name='Bob', age=20, score=88)
# 把Python对象变成一个JSON， 类似dump()方法可以直接把JSON写入一个file-like Object
json.dumps(d)
...
> 由于Pool的默认大小是CPU的核数。如果想自定义进程数：p = Pool(5)
...
> 可以理解为全局变量local\_school是一个dict，不但可以用local\_school.student，还可以绑定其他变量，如local_school.teacher等等。
...
> 请注意，当我们在一台机器上写多进程程序时，创建的Queue可以直接拿来用，但是，在分布式多进程环境下，添加任务到Queue不可以直接对原始的task\_queue进行操作，那样就绕过了QueueManager的封装，必须通过manager.get\_task_queue()获得的Queue接口添加。
